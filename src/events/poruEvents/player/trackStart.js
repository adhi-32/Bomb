...
module.exports.run = async (client, player, track) => {
    ...
    const Started = new EmbedBuilder()
        .setAuthor({
            name: `Now Playing`,
            iconURL: "https://cdn.discordapp.com/emojis/1107335646013562962.png?v=1&size=48&quality=lossless",
        })
    ...
    const filter = (message) => {
        if (message.guild.members.me.voice.channel && message.guild.members.me.voice.channelId === message.member.voice.channelId)
            ...
            message.reply({
                content: `<:wrong:1107334083404648589> | You must be on the same voice channel as mine to use this button.`,
                ephemeral: true,
            });
        }
    };
    ...
    collector.on("collect", async (message) => {
        if (message.customId === "loop") {
            ...
            if (!player) {
                ...
                const embed = new EmbedBuilder().setColor(client.color).setDescription(`<:wrong_axiom:1118527849863843890> | Song can't be replay`);
            ...
            if (!player) {
                ...
                const embed = new EmbedBuilder().setDescription(`<:wrong:1107334083404648589> | Queue is: \`Empty\``).setColor(client.color);
            ...
            if (!player) {
                ...
                const embed = new EmbedBuilder().setDescription(`<:wrong:1107334083404648589> | Previous song was: \`Not found\``).setColor(client.color);
            ...
            if (!player) {
                ...
                const embed = new EmbedBuilder().setDescription(`<:wrong:1107334083404648589> | Volume can't be lower than: \`10%\``).setColor(client.color);
            ...
            if (!player) {
                ...
                const embed = new EmbedBuilder().setDescription(`<:wrong_axiom:1118527849863843890> | Volume can't be higher than: \`100%\``).setColor(client.color);
