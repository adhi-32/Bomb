...
const bomb = client.channels.cache.get('1126844446936404019');

const cmdlog = new EmbedBuilder()
.setAuthor({ name: 'Command Using', iconURL: client.user.displayAvatarURL()})
.addFields([
{name: "Command Name", value: `/${command.name}`, inline: true},
{name: "Used By", value: `${interaction.user.tag}`, inline: true},
{name: "Used Server", value: `${interaction.guild.name}`, inline: true},
{name: "Server ID", value: `${interaction.guild.id}`, inline: true}
])
.setColor("#00FFF5")

bomb.send({embeds:[cmdlog]});
